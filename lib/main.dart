...
void main() async {
  ...
  // shared_preferences 인스턴스 생성 (SharedPreferences = 좋아요 정보를 키,밸류 형식으로 내장 메모리에 저장)
...
class CatService extends ChangeNotifier {
  ...
  void getRandomCatImages() async {
    Response resuld = await Dio().get(
        "https://api.thecatapi.com/v1/images/search?limit=10&mime_types=jpg");

    for (int i = 0; i < resuld.data.length; i++) {
      var map = resuld.data[i];
      catImages.add(map['url']);
    }
    notifyListeners(); // 컨슈머 아래 캣서비스 전체 새로고침
  }
